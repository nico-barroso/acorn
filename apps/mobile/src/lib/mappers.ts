...
export function mapResource(row: ResourceRow, tagColorMap: Map<string, string | null>): ContentCardData {
...
export function mapSearchResult(row: SearchRow, tagColorMap: Map<string, string | null>): SearchResult {
  const isFile = row.type === 'file';
...
export function mapFolderResource(row: ResourceRow, tagColorMap: Map<string, string | null>): FolderResource {
  const isFile = row.type === 'file';
  const fileUrl = row.url ?? undefined;
  return {
    id: row.id,
