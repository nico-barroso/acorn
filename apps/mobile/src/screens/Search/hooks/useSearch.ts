...
export function useSearch() {
  ...
  const { data: serverCount = 0 } = useQuery({
    ...
    queryFn: () =>
      fetchSearchCount(userId!, isTagQuery ? '' : debouncedQuery, filters, effectiveTag),
    enabled: Boolean(userId),
    staleTime: 30 * 1000,
  });

  const allUserTags = React.useMemo(() => (allTagsData ?? []).map((t) => t.name), [allTagsData]);
  ...
  const results = React.useMemo(() => infiniteData?.pages.flatMap((p) => p) ?? [], [infiniteData]);
  ...
  const handleToggleRead = React.useCallback(
    async (itemId: string, nextRead: boolean) => {
      const queryKey = [
        'search',
        userId,
        debouncedQuery,
